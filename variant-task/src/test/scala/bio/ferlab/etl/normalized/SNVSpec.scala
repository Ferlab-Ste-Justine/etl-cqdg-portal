  ...
  it should "generate NormalizedSNV from input raw VCF" in {
    val dataFomVCFFile: Map[String, DataFrame] = Map(
      raw_variant_calling.id -> Seq(
        ...
        VCF_SNV_INPUT(
          `contigName` = "chr2",
          `INFO_FILTERS` = Seq("DRAGENSnpHardQUAL"),
          `genotypes` = List(GENOTYPES(`sampleId` = "S4"))
        ) // Should be filtered out
      ).toDF(),
      specimenEnriched.id -> specimenEnrichedDf,
      normalized_task.id -> Seq(
        NORMALIZED_TASK(`study_id` = "STU0000001", `ldm_sample_id` = "S1", `experimental_strategies` = Seq("WGS")),
        NORMALIZED_TASK(`study_id` = "STU0000001", `ldm_sample_id` = "S2", `experimental_strategies` = Seq("WGS")),
        NORMALIZED_TASK(`study_id` = "STU0000001", `ldm_sample_id` = "S3", `experimental_strategies` = Seq("WXS"))
      ).toDF()
    )

    val results =
      SNV(TestETLContext(), "STU0000001", "owner", "dataset_default", "annotated_vcf", None).transform(dataFomVCFFile)
