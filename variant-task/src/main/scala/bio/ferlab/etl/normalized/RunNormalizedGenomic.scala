...
object RunNormalizedGenomic {
  ...
  def snv(rc: RuntimeETLContext,
          ...
          @arg(name = "reference-genome-path", short = 'g', doc = "Reference Genome Path") referenceGenomePath: Option[String]): Unit = SNV(rc, studyCode, owner, dataset, batch, referenceGenomePath).run()
