/** Generated by [[bio.ferlab.datalake.testutils.ClassGenerator]] on 2023-08-11T19:42:38.032861
  */
package models

case class NORMALIZED_TASK(
    `fhir_id`: String = "SXP0000001",
    `owner`: String = "CQDG",
    `bio_informatic_analysis`: Option[String] = None,
    `lab_aliquot_ids`: Seq[String] = Seq("1000004041-A11"),
    `ldm_sample_id`: String = "S16523",
    `run_ids`: Seq[String] = Seq("2018"),
    `run_dates`: Seq[String] = Seq("2007-04-14"),
    `selection`: CODEABLE = CODEABLE("RR"),
    `source`: CODEABLE = CODEABLE("TSC"),
    `protocol`: String = "protocol2",
    `target_capture_kit`: String = "targetCaptureKit3",
    `target_loci`: String = "targetedLoci2",
    `pipelines`: Seq[String] = Seq("testPipeline1", "testPipeline2"),
    `read_length`: Option[String] = Some("151"),
    `is_paired_end`: Option[Boolean] = Some(true),
    `capture_kit`: Option[String] = None,
    `platform`: Seq[String] = Seq("Illumina"),
    `instrument_model`: Seq[String] = Seq(),
    `pore_type`: Option[String] = None,
    `is_imputed`: Option[Boolean] = None,
    `experimental_strategy_1`: Seq[CODEABLE] = Seq(CODEABLE("WGS")),
    `experimental_strategy`: Seq[String] = Seq("WGS"),
    `profiling_resolution`: Seq[String] = Seq(),
    `sequencer_id`: Option[String] = None,
    `genome_build`: String = "GRCh38",
    `_for`: String = "PRT0000002",
    `analysis_files`: Seq[ANALYSIS_FILE] = Seq(
      ANALYSIS_FILE("Annotated-SNV", "FIL0000101"),
      ANALYSIS_FILE("Aligned-Reads", "FIL0000102"),
      ANALYSIS_FILE("SNV", "FIL0000104"),
      ANALYSIS_FILE("CNV", "FIL0000106"),
      ANALYSIS_FILE("SV", "FIL0000108"),
      ANALYSIS_FILE("Quality-Control-Metrics", "FIL0000605"),
      ANALYSIS_FILE("Raw-Sequencing-Reads", "FIL0000515"),
      ANALYSIS_FILE("Raw-Sequencing-Reads", "FIL0000516")
    ),
    `study_id`: String = "study1"
)

case class ANALYSIS_FILE(
    data_type: String,
    file_id: String
)
