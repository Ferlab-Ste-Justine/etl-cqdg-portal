...
//    }))
    ...
    patients.filter(_.`fhir_id` == "PRT0000004").head shouldEqual NORMALIZED_PATIENT()
    normalizedDiagnosis.filter(_.`subject` == "PRT0000004").head shouldEqual NORMALIZED_DIAGNOSIS()
    ...
    normalizedSampleRegistration.filter(_.`subject` == "PRT0000004").head shouldEqual NORMALIZED_SAMPLE_REGISTRATION()
