...
object PublishTask {
  ...
  def publish(
    ...
    val desiredIndices =
      retrieveIndexesFromRegex(generateRegexDesiredIndex(jobs, releaseId, studyList), "indices")(esNodes, esPort)
            ...
            studies.map(study => {
              Result(
                ...
                Try {
                  ...
                  if (
                    desiredIndices.exists(e => e.startsWith(s"${job}_$study")) &&
                  ...
                  desiredIndices
                    .find(e => e.startsWith(s"${job}_$study"))
                    .map(e => {
