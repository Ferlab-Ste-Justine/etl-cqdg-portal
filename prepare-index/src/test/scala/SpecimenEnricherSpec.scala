...
import model._
import model.input.{BIOSPECIMEN_INPUT, CODE_SYSTEM_DISPLAY_INPUT, SAMPLE_INPUT}
  ...
  "transform" should "enrich specimen" in {
    val data: Map[String, DataFrame] = Map(
      ...
      "normalized_family_relationship" -> Seq(
        FAMILY_RELATIONSHIP_NEW(
          internal_family_relationship_id = "FAMRO1",
      ...
      "normalized_sample_registration" -> Seq(
        SAMPLE_INPUT(
          `fhir_id` = "FHIR_SAMPLE1",
          subject = "P1",
          parent = "FHIR_BS_1",
          `submitter_sample_id` = "SAMPLE1"
        ),
        SAMPLE_INPUT(
          `fhir_id` = "FHIR_SAMPLE2",
          `subject` = "P2",
          parent = "FHIR_BS_2",
          `submitter_sample_id` = "SAMPLE2"
        )
      ).toDF(),
      "normalized_biospecimen" -> Seq(
        ...
        BIOSPECIMEN_INPUT(
          fhir_id = "FHIR_BS_3",
          subject = "P3",
          `submitter_biospecimen_id` = "BS_3",
          biospecimen_tissue_source = CODE_SYSTEM_DISPLAY_INPUT(`code` = "Unknown")
        )
      ).toDF(),
    ...
    specimensEnriched.find(_.`biospecimen_id` == "FHIR_BS_1") shouldBe Some(
      SPECIMEN_ENRICHED(
        `biospecimen_id` = "FHIR_BS_1",
        `age_biospecimen_collection` = "Young",
        `submitter_biospecimen_id` = "BS_1",
        `participant_id` = "P1",
        ...
        `sample_id` = "SAMPLE1",
        `fhir_sample_id` = "FHIR_SAMPLE1",
        is_a_proband = Some(true),
        `mother_id` = Some("P3"),
        `father_id` = Some("P2")
      )
    )
    specimensEnriched.find(_.`biospecimen_id` == "FHIR_BS_2") shouldBe Some(
      SPECIMEN_ENRICHED(
        `age_biospecimen_collection` = "Young",
        `is_affected` = Some(false)
      )
    )
    specimensEnriched.find(_.`biospecimen_id` == "FHIR_BS_3") shouldBe Some(
      SPECIMEN_ENRICHED(
        `biospecimen_id` = "FHIR_BS_3",
        `age_biospecimen_collection` = "Young",
        `submitter_biospecimen_id` = "BS_3",
        `biospecimen_tissue_source` = "Unknown",
        `participant_id` = "P3",
        `participant_fhir_id` = "P3",
        `submitter_participant_id` = "P3_internal",
        `is_affected` = Some(false),
        `sample_id` = null,
        `sample_type` = null,
        `fhir_sample_id` = null
