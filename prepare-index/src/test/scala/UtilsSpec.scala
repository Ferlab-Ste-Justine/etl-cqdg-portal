...
class UtilsSpec extends AnyFlatSpec with Matchers with WithSparkSession {
  ...
  "addFamily" should "add families to patients" in {
    val inputFamilies = Seq(
      ...
      GROUP(
        internal_family_id = "Family2",
        family_members = Seq("P4"),
        family_type = "solo",
        submitter_family_id = "fam1"
      )
    ).toDF()

    val inputFamilyRelationship = Seq(
      FAMILY_RELATIONSHIP(
        `internal_family_relationship_id` = "FAM_REL1",
        `category` = "category",
        `submitter_participant_id` = "P1",
        `focus_participant_id` = "P2",
        `relationship_to_proband` = "Father"
      ),
      FAMILY_RELATIONSHIP(
        `internal_family_relationship_id` = "FAM_REL2",
        `category` = "category",
      ...
      FAMILY_RELATIONSHIP(
        `internal_family_relationship_id` = "FAM_REL3",
        `category` = "category",
    ...
    val probands = output
      .select("participant_id", "is_a_proband")
      .where(col("is_a_proband"))
      .drop("is_a_proband")
      .as[String]
      .collect()
//    probands should contain theSameElementsAs Seq("P2", "P4") //FIXME what happen when a participant does not have family relationship (is he a proband)
    ...
    val patients = output
      .select("participant_id", "family_relationships")
      .as[(String, Seq[FAMILY_RELATIONSHIP_WITH_FAMILY])]
      .collect()
  ...
  "addFilesWithBiospecimen" should "only return participants with files" in {
    ...
    val seqExperimentDF = Seq
      .empty[SEQUENCING_EXPERIMENT_SINGLE]
      .toDF()
  ...
  "addSequencingExperiment" should " map the correct type_of_sequencing" in {
    ...
    output.select("sequencing_experiment.type_of_sequencing").as[String].collect() should contain theSameElementsAs Seq(
      "Paired Reads",
      "Unpaired Reads"
    )
  }
  ...
  "combineDataCategoryFromFilesAndStudy" should "add DataCategories listed from Study to Data Categories compiled from files" in {
    ...
    val study1Df = Seq(
      RESEARCHSTUDY(
        `study_id` = "study1",
        `data_categories` = Seq("data_category1", "data_category2", "data_category3")
      )
    ).toDF()
