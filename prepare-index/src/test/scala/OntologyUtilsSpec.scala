...
class OntologyUtilsSpec extends AnyFlatSpec with Matchers with WithSparkSession {
  ...
  "getTaggedPhenotypes" should "return tagged phenotypes and tagged phenotypes with ancestors" in {
    ...
    taggedPhenotypes shouldBe
      ("1", Seq(
        PHENOTYPE_TAGGED(`internal_phenotype_id` = "1", `is_leaf` = true, `name` = "G Name (HP:G)", `parents` = Seq("B Name (HP:B)"), `age_at_event` = 0),
        PHENOTYPE_TAGGED(`internal_phenotype_id` = "2", `is_leaf` = true, `name` = "E Name (HP:E)", `parents` = Seq("B Name (HP:B)", "C Name (HP:C)"), `age_at_event` = 0),
      ))
    ...
    notTaggedPhenotypes shouldBe
      ("1", Seq(
        PHENOTYPE_TAGGED(`internal_phenotype_id` = "3", `parents` = Seq("A Name (HP:A)"), `name` = "C Name (HP:C)"),
      ))
