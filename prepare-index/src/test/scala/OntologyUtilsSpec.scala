...
import model.{
  ...
  PHENOTYPE_HPO_CODE,
  PHENOTYPE_TAGGED,
  PHENOTYPE_TAGGED_WITH_ANCESTORS,
...
class OntologyUtilsSpec extends AnyFlatSpec with Matchers with WithSparkSession {
  ...
  "getTaggedPhenotypes" should "return tagged phenotypes and tagged phenotypes with ancestors" in {
    ...
    val phenotype1 = PHENOTYPE(
      `fhir_id` = "1",
      `phenotype_source_text` = "text",
      `phenotype_HPO_code` = PHENOTYPE_HPO_CODE(`code` = "HP:G"),
      `cqdg_participant_id` = "1",
      `age_at_phenotype` = Some("Young")
    )
    val phenotype2 = PHENOTYPE(
      `fhir_id` = "2",
      `phenotype_source_text` = "text",
      `phenotype_HPO_code` = PHENOTYPE_HPO_CODE(`code` = "HP:E"),
      `cqdg_participant_id` = "1",
      `age_at_phenotype` = Some("Old")
    )
    val phenotype3 = PHENOTYPE(
      `fhir_id` = "3",
      `phenotype_source_text` = "text",
      `phenotype_HPO_code` = PHENOTYPE_HPO_CODE(`code` = "HP:C"),
      `cqdg_participant_id` = "1",
      `phenotype_observed` = "NEG",
      `age_at_phenotype` = Some("Super Old")
    )
    ...
    taggedPhenotypes shouldBe
      (
        ...
        Seq(
          PHENOTYPE_TAGGED_WITH_OBSERVED(
            `internal_phenotype_id` = "1",
            `is_leaf` = true,
            `name` = "G Name (HP:G)",
            `parents` = Seq("B Name (HP:B)"),
            `age_at_event` = Some("Young"),
            `source_text` = "text",
            `is_observed` = Some(true)
          ),
          PHENOTYPE_TAGGED_WITH_OBSERVED(
            `internal_phenotype_id` = "2",
            `is_leaf` = true,
            `name` = "E Name (HP:E)",
            `parents` = Seq("B Name (HP:B)", "C Name (HP:C)"),
            `age_at_event` = Some("Old"),
            `source_text` = "text",
            `is_observed` = Some(true)
          ),
          PHENOTYPE_TAGGED_WITH_OBSERVED(
            `internal_phenotype_id` = "3",
            `parents` = Seq("A Name (HP:A)"),
            `name` = "C Name (HP:C)",
            `age_at_event` = Some("Super Old"),
            `source_text` = "text",
            `is_observed` = Some(false)
          )
        )
      )
    ...
    taggedPhenotypesWithAncestors._2 should contain theSameElementsAs
      Seq(
        ...
        PHENOTYPE_TAGGED_WITH_ANCESTORS(
          ...
          `is_tagged` = true,
          `parents` = Seq("B Name (HP:B)"),
          `age_at_event` = Seq("Young"),
          `name` = "G Name (HP:G)"
        ),
        PHENOTYPE_TAGGED_WITH_ANCESTORS(
          `parents` = Seq("A Name (HP:A)"),
          `age_at_event` = Seq("Old", "Young"),
          `name` = "B Name (HP:B)"
        ),
  ...
  it should "return replace phenotypes that are obsolete" in {
    ...
    val phenotype2 = PHENOTYPE(
      `fhir_id` = "2",
      `phenotype_HPO_code` = PHENOTYPE_HPO_CODE(`code` = "HP:D"),
      `cqdg_participant_id` = "1",
      `phenotype_source_text` = "text"
    )
  ...
  "getDiagnosis" should "return diagnosis per participant" in {
    ...
    val diagnosis1 = DIAGNOSIS_INPUT(
      `subject` = "PRT0000001",
      `fhir_id` = "DIA0000001",
      `diagnosis_source_text` = "text1",
      `diagnosis_mondo_code` = "HP:G",
      `diagnosis_ICD_code` = "HP:G"
    )
    val diagnosis2 = DIAGNOSIS_INPUT(
      `subject` = "PRT0000002",
      `fhir_id` = "DIA0000002",
      `diagnosis_source_text` = "text2",
      `diagnosis_mondo_code` = "HP:E",
      `diagnosis_ICD_code` = "HP:E"
    )
    val diagnosis3 = DIAGNOSIS_INPUT(
      `subject` = "PRT0000001",
      `fhir_id` = "DIA0000003",
      `diagnosis_source_text` = "text3",
      `diagnosis_mondo_code` = "HP:B",
      `diagnosis_ICD_code` = "HP:B"
    )
  ...
  "getDiagnosis" should "map ICD terms with '.' or '-' inside" in {
    ...
    val diagnosis2 =
      DIAGNOSIS_INPUT(`fhir_id` = "DIA0000002", `diagnosis_ICD_code` = "A20-A28") // as A20-A28 in input file
