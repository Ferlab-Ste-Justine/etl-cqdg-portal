...
class ParticipantCentricSpec
  ...
  "transform" should "prepare index participant_centric" in {
    val data: Map[String, DataFrame] = Map(
      ...
      "ncit_terms" -> read(
        getClass.getResource("/ncit_terms").toString,
        "Parquet",
        Map(),
        None,
        None
    ...
    val output =
      new ParticipantCentric(List("SD_Z6MWD3H0"))(conf).transform(data)

    output.keys should contain("es_index_participant_centric")

    val participant_centric =
      output("es_index_participant_centric").as[PARTICIPANT_CENTRIC].collect()
